...
let create (generators : Permutation.t list) : t =
  ...
  | [] ->
      invalid_arg
        "PermutationGroup.create: cannot infer a degree from an empty generating set"
  | first :: _ ->
...
let elements (group : t) : Permutation.t list =
  ...
  Seen.add seen identity ();
  Queue.add identity queue;
  while not (Queue.is_empty queue) do
    let permutation = Queue.take queue in
    List.iter
      (fun generator ->
        let product = Permutation.comp permutation generator in
        if not (Seen.mem seen product) then begin
          Seen.add seen product ();
          Queue.add product queue
        end)
      group.generators
  done;
  Seen.fold (fun permutation () acc -> permutation :: acc) seen []
